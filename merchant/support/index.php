...
require_once __DIR__ . '/_ticket_logic.php';
...
$stmt = $pdo->prepare("SELECT role, full_name, account_status, party_code AS merchant_id, business_name FROM users WHERE id = ?");
...
if ($active) {
    render_merchant_shell_open($pdo, $user_id, 'support', 'Customer Support & Tickets');
} else {
    render_shell_open($pdo, $user_id, '', 'Support');
}
?>
...
<style>
    ...
    .empty-state { background: var(--surface); border: 1px dashed var(--line); border-radius: 12px; padding: 30px 26px; text-align: center; color: var(--muted); font-size: 13px; }
...
<div class="page-wrap">
    <div class="kb-banner">
        ...
        <div>
            ...
            <div class="kb-sub">Onboarding, payments, settlements, credit and more — search our knowledge base first.</div>
        </div>
        <a class="btn-primary-sm" href="knowledge_base.php"><i class="fa-solid fa-magnifying-glass"></i> Browse knowledge base</a>
    </div>
    ...
    <div class="section-card">
        ...
        <div class="tabs">
            <?php foreach ($ticketTabs as $key => $label): ?>
                <a href="index.php?status=<?= urlencode($key) ?>" class="tab <?= $key === $activeTab ? 'active' : '' ?>">
        ...
        <?php if (empty($tickets)): ?>
            <div class="empty-state"><?= $activeTab === 'all' ? 'No support tickets yet. Need help? Start with the knowledge base or raise a ticket above.' : 'No tickets with this status.' ?></div>
        <?php else: ?>
