-- =====================================================================
-- INDBIN CRM : complete database, all three modules plus integration
--
-- Paste into the phpMyAdmin SQL tab and press Go. Replaces every earlier
-- SQL file (schema.sql, install_localhost.sql, migrate_existing.sql).
--
-- ---------------------------------------------------------------------
-- READ FIRST
--
-- This DROPS and rebuilds everything, users included. Export anything you
-- want to keep before running it: Export > Custom > pick tables > Go.
--
-- Columns and table names below were taken from the queries the merchant
-- and agent modules actually run, not invented. Neither module ships a
-- schema, so their tables have never formally existed; this is the first
-- definition of them.
--
-- ---------------------------------------------------------------------
-- THE INTEGRATION DECISION
--
-- The three modules each had their own idea of an application. Merchant
-- and agent shared `applications`; the customer module had its own
-- `customer_applications`. A customer raising a requirement and a merchant
-- converting a lead therefore produced two unrelated rows, so no agent
-- could see the customers request and no commission could be traced.
--
-- There is now ONE `applications` table carrying nullable customer_id,
-- agent_id and merchant_id. Whoever is involved is on the row. That single
-- change is what turns three sites sharing a database into one CRM.
-- =====================================================================
...
DROP VIEW  IF EXISTS products, customer_applications;
DROP TABLE IF EXISTS
    audit_logs, notifications, messages, support_tickets,
    commission_payouts, commissions, agent_commission_payout, agent_wallet,
    agent_hierarchy, agent_training, agent_verification, background_verifications,
    training_progress, agreement_signatures, payout_preferences, settlement_accounts,
    merchant_services, self_service_applications, transactions, repayments,
    quotes, application_events, applications,
    leads, customers, product_activations, product_catalog,
    credit_evaluations, credit_assessments, business_verifications,
    consents, bank_mandates, payment_instruments, customer_profiles,
    loyalty_ledger, loyalty_enrollments, referrals,
    customer_documents, kyc_documents, kyc_details, ekyc,
    otp_verifications, customer_onboarding, admins, users;
...
SET party_code = CONCAT(
  ...
  CASE role
    ...
    ELSE 'U'   -- fallback for any role not listed above, adjust as needed
  END,
  LPAD(id, 7, '0')
)
-- =====================================================================
...
ALTER TABLE users
ADD COLUMN assigned_role VARCHAR(100) NULL;
-- =====================================================================
-- 12. CHECK
--     Expect 43 tables and 2 views.
-- =====================================================================
...
LEFT JOIN customers c
    ON c.id = ca.customer_id
LEFT JOIN users u
    ON u.id = ca.agent_user_id
ORDER BY ca.id DESC;
-- =====================================================================
-- Training lesson content — 2 pages per topic, real written content
-- Safe to run: CREATE TABLE IF NOT EXISTS, nothing else is touched.
-- =====================================================================
...
INSERT INTO training_content (topic_key, page_number, heading, body, tip) VALUES

('accept_payments', 1, 'How customers pay you',
'Every INDBIN merchant account comes with a UPI-linked QR code. When a customer scans it with any UPI app — PhonePe, Google Pay, Paytm, or their bank''s own app — the payment goes straight into your linked settlement account.\n\nThere is no card machine to buy and no setup fee. The QR code is generated automatically the moment your account is activated, and it stays the same unless you request a change from Support.',
...
'You can find your QR code anytime under Manage Products & Services on your dashboard, where it can be downloaded as an image or reprinted.\n\nEvery payment you receive shows up in Orders & Transactions within a few seconds, along with the customer''s UPI handle and the exact amount. If a payment doesn''t appear within a minute, ask the customer to check their UPI app for a failed or pending status before trying again — this avoids accidental double charges.',
...
'Your credit limit is set after your business verification is reviewed, based on your turnover, years in business, and the documents you submitted. You can see your current limit and risk category anytime on the Credit & Risk Assessment page.\n\nThis limit determines how much value you can extend to customers through INDBIN''s pay-later options at your shop. It is not a loan to you personally — it is the ceiling on customer credit your business is approved to offer.',
...
'When a customer uses pay-later credit at your shop, INDBIN settles the full amount to you upfront — you are paid immediately, the same as a normal transaction. The customer then repays INDBIN directly over their agreed schedule.\n\nYou never need to chase a customer for repayment yourself. If a customer misses a payment, that is handled entirely between them and INDBIN''s collections process, and it does not affect your settlement, which already happened.',
'You can see which of your sales used pay-later credit under Repayment Tracking — useful for spotting your most credit-active regular customers.'),

('settlements', 1, 'What a settlement cycle means',
'A settlement is the transfer of money from payments you''ve received into your actual bank account. You chose your settlement frequency — daily, weekly, or monthly — during Account Setup, and you can change it anytime from the same page.\n\nMost merchants start with weekly settlements to get a feel for the flow, then switch to daily once their transaction volume grows enough to make it worthwhile.',
'Daily settlement has no extra fee on INDBIN — there''s no downside to switching to it once you''re comfortable.'),

('settlements', 2, 'Tracking your payouts',
'Every settlement is logged under Repayment Tracking with the exact amount transferred, the bank account it went to, and the date. If a settlement is delayed past your usual cycle, check there first — it will show as "processing" rather than silently missing.\n\nIf you ever change your bank account details, existing pending settlements will still go to the account that was active at the time the payment was received, not the newly updated one.',
'Screenshot or export your settlement history monthly if you reconcile your own books manually — it saves time during tax season.'),

('dashboard', 1, 'Finding your way around',
'Your dashboard sidebar is organized around the actual flow of running your business here: Leads come in, you send Quotes, they become Applications, and successful ones become Orders. Repayments and Support sit below that, and Analytics ties it all together.\n\nThe home screen always shows your most recent activity across all of these, so you rarely need to hunt through each section individually just to see what happened today.',
'Bookmark the dashboard home page — it''s built to answer "what happened since I last checked" faster than any individual sub-page.'),

('dashboard', 2, 'Reports that matter',
'Analytics & Business Reports shows your transaction volume, top products, and customer repeat-rate over whatever time range you pick. These numbers update in real time, not on a delay.\n\nIf a number looks wrong — a total that doesn''t match what you remember taking in — check the date range filter first. The most common cause is an accidentally narrow date range, not a data error.',
...
('services', 1, 'What''s enabled on your account',
'Manage Products & Services shows every product INDBIN offers and which ones are currently active for your account. Some, like basic UPI acceptance, are on by default. Others need to be requested and approved.\n\nAn enabled service doesn''t cost anything extra just for having it turned on — costs only apply when it''s actually used, and those are always shown before you confirm anything.',
'Check this page after any major update — INDBIN periodically enables new no-cost services by default, and it''s easy to miss the announcement.'),

('services', 2, 'Requesting something new',
'If a service you want isn''t active yet, use Apply for Loan / Insurance or the relevant request button on the Services page. Most approvals happen automatically based on your existing verification — you usually won''t need to resubmit documents you''ve already provided.\n\nIf a request is declined, the reason is always shown, and you can usually reapply once the underlying issue (like an out-of-date document) is resolved.',
